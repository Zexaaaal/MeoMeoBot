...
function registerHandlers(deps) {
        ...
        if (config.currentTheme) {
            ...
            try {
                if (fs.existsSync(themePath)) {
                    ...
                    logger.log(`[RESET] Deleted stale theme file: ${themePath}`);
                }
            } catch (e) {
                logger.error(`[RESET] Error deleting theme file: ${e.message}`);
            }
        }
    ...
    ipcMain.handle('save-reward-sound', (event, rewardId, soundPath) => {
        const config = bot.getConfig();
        const rewardSounds = config.rewardSounds || {};
        if (soundPath) {
            rewardSounds[rewardId] = soundPath;
        } else {
            delete rewardSounds[rewardId];
        }
        bot.updateConfig({ rewardSounds });
        return { success: true };
    });
    ...
    ipcMain.handle('save-reward-function', (event, rewardId, functionName) => {
        const config = bot.getConfig();
        const rewardFunctions = config.rewardFunctions || {};
        if (functionName) {
            rewardFunctions[rewardId] = functionName;
        } else {
            delete rewardFunctions[rewardId];
        }
        bot.updateConfig({ rewardFunctions });
        return { success: true };
    });
    ...
    ipcMain.handle('save-reward-image', (event, rewardId, imagePath) => {
        const config = bot.getConfig();
        const rewardImages = config.rewardImages || {};
        if (imagePath) {
            rewardImages[rewardId] = imagePath;
        } else {
            delete rewardImages[rewardId];
        }
        bot.updateConfig({ rewardImages });
        return { success: true };
    });
    ...
    ipcMain.handle('save-planning-base64', async (event, dataURL) => {
        ...
        try {
            ...
            logger.error('Error saving planning image:', e);
