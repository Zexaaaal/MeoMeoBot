...
class SubgoalsWidgetServer extends BaseWidgetServer {
    constructor(bot, defaultPort) {
        ...
        if (this.bot) {
            this.bot.onLastSub = (data) => this.broadcastLastEvent('last-sub', data);
            this.bot.onLastFollow = (data) => this.broadcastLastEvent('last-follow', data);
            this.bot.onLastDonation = (data) => this.broadcastLastEvent('last-donation', data);
        }
    }
